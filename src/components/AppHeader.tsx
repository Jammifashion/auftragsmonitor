...
export default function AppHeader({ onMenuClick }: AppHeaderProps) {
  const { user } = useAuth();
  return (
    <header className="h-16 bg-slate-950 flex items-center justify-between px-4 md:px-8 border-b border-slate-900 shrink-0">
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <button className="p-2 ml-1 text-slate-400 hover:text-white hover:bg-slate-800 rounded-full transition-colors md:hidden" onClick={onMenuClick}>
